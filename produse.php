<?php
$title = 'LorMan Di Mano - Produse din piele / Magazin Virtual';
...
$rezultate_maxime = 14;
$intrari_totale = mysqli_num_rows(mysqli_query($conexiune, 'SELECT `id` FROM `produse` WHERE `cat`="' . $_GET['cat'] . '" AND `subcat`="' . $_GET['subcat'] . '"'));
...
if ($intrari_totale == 0) {
	...
	$cerereSQL = 'SELECT * FROM `produse` WHERE `cat`="' . $_GET['cat'] . '" AND `subcat`="' . $_GET['subcat'] . '" ORDER BY `id` DESC LIMIT ' . $from . ', ' . $rezultate_maxime . '';
	...
	if ($pagina > $pagini_totale) echo 'Pagina nu exista !';
		...
		if ($pagini_totale > 0) {
			...
			if ($_GET['cat'] == 'Femei') $title = 'Produse Dama';
			elseif ($_GET['cat'] == 'Barbati') $title = 'Produse Barbati';
			elseif ($_GET['cat'] == 'Bussines') $title = 'Produse Business';
			else $title = 'Produse';
			...
			while ($rand = mysqli_fetch_assoc($rezultat)) {
				...
				echo '<td align="center" width="50%"><table border="0" cellspacing="3"><tr><td width="100" align="center" valign="middle"><a href="vezi_produs.php?id=' . $rand['id'] . '"><img src="' . $imgsrc . '" width="100" alt="" class="prod"></a></td>
							...
							<font color="red"><b>' . $rand['pret'] . ' RON / ' . $euro . ' &euro;</b></font></td></tr></table></td>';
